...
Read `<file>`, parse the XML, validate every `<note>`, and post the
valid ones to Anki via AnkiConnect.
...
even though the XML is structurally correct. You'd then have to
open Anki and create the deck by hand, or shell out to
AnkiConnect's `createDeck` from somewhere else.
...
create `A`, `A::B`, and `A::B::C` in one call. Our tool always
calls `createDeck` per unique deck name (deduplicated), so you
get exactly the set of decks the file references.
...
behavior is irrelevant — the tool just reports what *would* have
been sent.
